...
#結果保存ファイルを生成
filename = 'n_cluster_' + str(k) + '_wl40.txt'
...
#SVM with HMM likelihood, rbfのパラメータチューニング
for param1 in [0.01, 0.1, 1, 10, 100, 1000]:
	print 'C=%f' % param1
	resultfile.write(('C=%f' % param1) + '\n')
	for param2 in [0.0001, 0.001, 0.01, 0.0, 0.1, 1]:
		print 'gamma=%f' % param2
		resultfile.write(('gamma=%f' % param2) + '\n')

		svm_likelihood_rbf = svm.SVC(kernel='rbf', C=param1, gamma=param2)
...
for param1 in [0.01, 0.1, 1, 10, 100, 1000]:
	...
	for param2 in [0.0001, 0.001, 0.01, 0.0, 0.1, 1]:
		...
		svm_rawmocap_rbf = svm.SVC(kernel='rbf', C=param1, gamma=param2)
