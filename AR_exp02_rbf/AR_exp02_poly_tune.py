...
#SVM with HMM likelihood, polyのパラメータチューニング
for param1 in [0.01, 0.1, 1, 10, 100, 1000]:
	print 'C=%f' % param1
	resultfile.write(('C=%f' % param1) + '\n')
	for param2 in [1, 2, 3]:
		print 'degree=%f' % param2
		resultfile.write(('degree=%f' % param2) + '\n')

		svm_likelihood_poly = svm.SVC(kernel='poly', C=param1, degree=param2)
		svm_likelihood_poly.fit(features_train, labels_train)
		labels_predicted_svm_likelihood_poly = svm_likelihood_poly.predict(features_test)

		count = 0
		for i in range(len(labels_predicted_svm_likelihood_poly)):
		    if labels_predicted_svm_likelihood_poly[i] == labels_test_transformed[i]:
		        count += 1
		accuracy = count / np.float(len(labels_predicted_svm_likelihood_poly))        
		result = 'SVM with HMM likelihood, poly: accuracy=%f' % accuracy
		print result
		resultfile.write(result + '\n')
...
# #SVM raw mocap, polyのパラメータチューニング	
# for param1 in [0.01, 0.1, 1, 10, 100, 1000]:
# 	print 'C=%f' % param1
# 	resultfile.write(('C=%f' % param1) + '\n')
# 	for param2 in [1, 2, 3]:
...
# 		svm_rawmocap_poly = svm.SVC(kernel='poly', C=param1, degree=param2)
...
# 		result = 'SVM with raw mocap, poly: accuracy=%f' % accuracy
# 		print result
# 		resultfile.write(result + '\n')
