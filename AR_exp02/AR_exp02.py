...
#SVM with HMM likelihood, polynomial
svm_likelihood_poly = svm.SVC(kernel='poly')
...
result = 'SVM with HMM likelihood, polynomial: accuracy=%f' % accuracy
print result
resultfile.write(result + '\n')
...
svm_rawmocap_poly = svm.SVC(kernel='poly')
...
for i in range(len(labels_predicted)):
    if labels_predicted[i] == labels_svm_test[i]:
        count += 1
accuracy = count / np.float(len(labels_predicted))
result = 'SVM with raw mocap, polynomial: accuracy=%f' % accuracy
print result
resultfile.write(result + '\n')