...
#svmhmmのwindow_lengthは10, svmrawは1とする
wl = 10
print 'SVM-HMM, window_length: %d' % wl

#結果保存ファイルを生成
filename = 'svmhmm_tune.txt'
resultfile = open(filename, 'w')
resultfile.write('window_length_svmhmm = %d\n' % wl)
...
#SVM with HMM likelihood, rbfをグリッドサーチ
parameters = {'kernel':['rbf'], 'gamma':[0.0, 10e-3, 10e-4], 'C':np.logspace(-4, 4, 10)}
svr = svm.SVC()
svm_likelihood_rbf = grid_search.GridSearchCV(svr, parameters, cv=5 ,n_jobs=-1)
...
print svm_likelihood_rbf.best_estimator_
print result
resultfile.write(result + '\n')
resultfile.close()